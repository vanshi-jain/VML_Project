...
def get_dataset_and_loaders(args, shuffle_train=True, shuffle_val=True):
    ...
    train_loader, val_loader = ds.make_loaders(only_val=args.eval_only, batch_size=args.batch_size, 
                                    workers=1, shuffle_train=shuffle_train, shuffle_val=shuffle_val)
...
def get_model(args, ds):
    ...
    fit the target dataset) and a checkpoint. 
    The checkpoint is set to None if not resuming training.
    '''
    finetuned_model_path = os.path.join(
        args.out_dir, args.exp_name, args.resume_ckpt_name)
    print("this is fined-",finetuned_model_path)

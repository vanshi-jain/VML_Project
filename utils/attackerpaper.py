...
class PatchAttacker:
    def __init__(self, model, mean, std, kwargs):
        ...
        print("this is the attacker!",{
            'patch_size': self.patch_l,
            'eps': self.epsilon,
            'steps': self.steps,
    ...
    def perturb(self, inputs, labels, norm='l2', random_count=1):
        ...
        for _ in range(random_count):
            ...
            for step in range(self.steps):
                ...
                loss_ind = torch.nn.CrossEntropyLoss(reduction='none')(output[0], labels)
                ...
                loss = loss_ind.sum()
                grads = torch.autograd.grad(loss, [x])[0].detach()

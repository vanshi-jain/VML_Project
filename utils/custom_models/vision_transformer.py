...
from .preprocess import MaskProcessor
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        print("This is the patch embedding:",{
            "image size": img_size,
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, head_mask=None, debug_flags=None, patch_mask=None):
        ...
        x = self.head(x) # input is 192 patches(only col to train) and output is 10 classes
        if debug_flags is None:
            return x
        else:
            return x, debug
